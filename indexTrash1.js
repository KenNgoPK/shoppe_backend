...
//     try {
//         const { sortBy, order } = req.query;
//         const sql = `SELECT ProductId, ProductName, Price, Rating, Stock FROM PRODUCTS`;
...
//                 if ((order === 'ASC' && left[0][key] <= right[0][key]) || 
//                     (order === 'DESC' && left[0][key] >= right[0][key])) {
...
//         const sortedProducts = mergeSort(rows, sortBy, order);
//         res.status(200).json(sortedProducts);
//     } catch (error) {
//         res.status(500).json({ message: 'Lỗi xử lý dữ liệu', error });
//     }
// });
